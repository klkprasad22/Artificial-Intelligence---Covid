...
def lable_loading(label_file):
    ...
    fnames = ["/home/pramod/COVID-19_Detection/image/archive/2A_images/" + fname for fname in fnames]
    fnames = np.array(fnames)
    classes = np.array(classes)
    return fnames, classes, bboxes
def idx_gen(fnames , SET):
...
def data_frame_generator(test_index ,classes_test ):
    ...
    return df
def con_data(filepath, classes , dim_size ,index  ,bboxes , interpolation = cv2.INTER_AREA , intensify =False):
...
def labels_updater(dense_y):
    ...
    for i in range(len(dense_y)):
        if dense_y[i] == 2:
            ...
            dense_y[i] = 0 
    return dense_y    
def idx_extr(y_preds, y_test,  test_index ,classes_test):
    """
...
def c_matrix(cm, classes,normalize=False,title='Confusion matrix',cmap=plt.cm.Blues):
    """
